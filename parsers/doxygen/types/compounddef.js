...
function compounddefType(compounddef){
    ...
    if (compounddef.$('title').children.length){
        ...
        compound.summary.name = compounddef.$('title').text();
    } else {
        // Compound name (1)
        compound.body.name = compounddef.$('compoundname').text();
        compound.summary.name = compounddef.$('compoundname').text();
    }
    ...
    if (compounddef.$('briefdescription').children[0] &&
        compounddef.$('briefdescription').children[0].children.length){
        compound.summary.description = types.description(compounddef.$('briefdescription').children[0]);
        compound.body.description = types.description(compounddef.$('briefdescription').children[0]);
    }

    // Detailed description (0:1)
    if (compounddef.$('detaileddescription').children[0] &&
        compounddef.$('detaileddescription').children[0].children.length){
